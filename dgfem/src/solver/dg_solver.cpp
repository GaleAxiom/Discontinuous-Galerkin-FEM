...
DGSolverBase::DGSolverBase(std::shared_ptr<DGMesh> mesh) : mesh_(mesh) {}

// LaplaceDGSolver implementation
LaplaceDGSolver::LaplaceDGSolver(std::shared_ptr<DGMesh> mesh, double penalty_parameter)
    : DGSolverBase(mesh), weak_form_(std::make_shared<LaplaceWeakFormulation>(penalty_parameter)) {
...
AdvectionDGSolver::solve(std::function<double(const Eigen::Vector2d&)> initial_condition,
    ...
    std::cout << "  Starting time stepping (T_final=" << T_final << ", dt=" << dt << ")..."
              << std::endl;
    ...
    while (t < T_final) {
        if (step % save_every == 0 && step > 0) {
            ...
            if (step % (10 * save_every) == 0) {
                std::cout << "    t = " << t << " / " << T_final << "  (u_min=" << u.minCoeff()
                          << ", u_max=" << u.maxCoeff() << ")" << std::endl;
            }
        }
    ...
    std::cout << "Time stepping completed. Saved " << solution_frames.size() << " frames."
              << std::endl;
...
EulerDGSolver::EulerDGSolver(std::shared_ptr<DGMesh> mesh,
    ...
    : DGSolverBase(mesh), weak_form_(std::move(weak_form)), gamma_(weak_form_->get_gamma()) {
    assembler_ = std::make_shared<DGAssembler>(mesh_, weak_form_);
}
...
EulerDGSolver::apply_mass_inv(const std::vector<Eigen::MatrixXd>& vec) const {
    int n_elem = vec.size();
...
std::vector<Eigen::MatrixXd>
EulerDGSolver::assemble_euler_residual(const std::vector<Eigen::MatrixXd>& u_coeffs) const {
    return assembler_->assemble_euler_residual(u_coeffs);
}

std::vector<Eigen::MatrixXd>
EulerDGSolver::time_step_ssp_rk3(const std::vector<Eigen::MatrixXd>& u_n, double dt) const {
    // Use refactored SSP-RK3 from time_stepping utilities
...
EulerDGSolver::solve(std::function<Eigen::Vector4d(const Eigen::Vector2d&)> initial_condition,
    ...
    // 3. Time stepping loop
    std::cout << "  Starting time stepping (SSP-RK3, T_final=" << T_final << ", dt=" << dt << ")..."
              << std::endl;
    ...
    while (t < T_final) {
        ...
        if (step % 10 == 0) {
            ...
            std::cout << "    Step " << step << ", t = " << t << " / " << T_final << "  (rho: ["
                      << rho_min << ", " << rho_max << "], CFL: " << std::fixed
                      << std::setprecision(3) << current_cfl << ")" << std::endl;

            // Warn if CFL is getting large
            if (current_cfl > 1.0 && step % 50 == 0) {
                std::cout << "    WARNING: CFL = " << current_cfl << " > 1.0 at step " << step
                          << std::endl;
            }
        }
    ...
    std::cout << "Euler solve completed with " << solution_frames.size() << " solution frames."
              << std::endl;
    std::cout << "  Total time steps: " << step << std::endl;
    std::cout << "  Total time stepping: " << std::fixed << std::setprecision(4)
              << total_timestep_time << " s" << std::endl;
    std::cout << "  Avg time per step: " << std::fixed << std::setprecision(6)
              << (total_timestep_time / step) << " s" << std::endl;
    std::cout << "  Maximum CFL encountered: " << std::fixed << std::setprecision(4)
              << max_cfl_encountered << std::endl;
...
double EulerDGSolver::compute_max_cfl(const std::vector<Eigen::MatrixXd>& u_coeffs,
                                      double dt) const {
    int n_elem = mesh_->get_n_elements();
    int n_basis = mesh_->get_dg_space()->get_basis()->get_n_basis();
    double max_cfl = 0.0;
...
NavierStokesDGSolver::NavierStokesDGSolver(std::shared_ptr<DGMesh> mesh, double gamma,
    ...
    : EulerDGSolver(mesh, std::make_shared<NavierStokesWeakFormulation>(

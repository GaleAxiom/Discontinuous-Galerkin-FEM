...
std::vector<Eigen::MatrixXd> DGAssembler::assemble_euler_residual(
    ...
    for (int elem_id = 0; elem_id < n_elem; ++elem_id) {
        const auto& elem_data = mesh_->get_element_data(elem_id);
        residuals[elem_id] = euler_weak_form_->volume_residual(u_coeffs[elem_id], elem_data, dg_space_);
    }

    if (euler_weak_form_->has_viscous_terms()) {
        for (int elem_id = 0; elem_id < n_elem; ++elem_id) {
            const auto& elem_data = mesh_->get_element_data(elem_id);
            residuals[elem_id] += euler_weak_form_->viscous_volume_residual(u_coeffs[elem_id], elem_data, dg_space_);
        }
    }
    ...
    for (const auto& face : interior_faces) {
        ...
        auto [R_face_L, R_face_R] = euler_weak_form_->interior_face_residual(
            ...
            face_data_L, face_data_R, dg_space_, face.permutation);
    ...
    if (call_count % 300 == 0) {
        std::cout << "[TIMER] Residual assembly (" << call_count << " calls): "
                  << "Volume=" << std::fixed << std::setprecision(4) << total_vol_time << "s, "
                  << "Face=" << total_face_time << "s, "

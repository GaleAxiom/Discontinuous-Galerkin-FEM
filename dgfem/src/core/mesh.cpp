...
DGMesh::DGMesh(const DView2& vertices, const IView2& elements, const IView1& element_tags,
    ...
    std::cout << "Created DGMesh with " << n_elements_ << " " << element_type_ << " elements and "
              << boundary_faces_.size() << " boundary faces" << std::endl;
}
...
std::vector<std::pair<int, int>> DGMesh::get_element_neighbors(int elem_id) const {
    ...
    for (int face = 0; face < n_faces_per_elem_; ++face) {
        neighbors[face] = {face_neighbors_(elem_id, face * 2),
                           face_neighbors_(elem_id, face * 2 + 1)};
    }
    return neighbors;
}
...
std::shared_ptr<BoundaryConditionEuler> DGMesh::get_boundary_condition_euler(int elem_id,
                                                                             int face_id) const {
...
std::shared_ptr<BoundaryConditionEuler>
DGMesh::get_boundary_condition_euler(std::string_view tag_name) const {
    std::string tag_str(tag_name);
...
void DGMesh::identify_boundary_faces(
    ...
    // Identify all boundary faces and assign tags. Must run once, after sig_to_tag has been
    // populated from every boundary tag above -- this used to be nested inside the per-tag
    // loop, so the full element/face scan ran once per tag and appended duplicates each time
    // (e.g. 8 real boundary faces x 4 tags = 32 entries in boundary_faces_ for a rectangular
    // mesh).
    for (int elem_id = 0; elem_id < n_elements_; ++elem_id) {
        for (int face_id = 0; face_id < n_faces_per_elem_; ++face_id) {
...
void DGMesh::set_periodic_boundaries(std::string_view tag_name_1, std::string_view tag_name_2) {
    ...
    if (faces_1.empty() || faces_2.empty()) {
        throw std::runtime_error(
            "Cannot set periodic boundaries: one or both boundaries have no faces");
    }

    std::cout << "Setting up periodic boundaries between '" << tag_str_1 << "' (" << faces_1.size()
              << " faces) and '" << tag_str_2 << "' (" << faces_2.size() << " faces) ..."
              << std::endl;
    ...
    std::cout << "  Set up " << periodic_face_map_.size() / 2 << " periodic face pairs"
              << std::endl;

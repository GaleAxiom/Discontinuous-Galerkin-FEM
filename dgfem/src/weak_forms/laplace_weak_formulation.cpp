...
double LaplaceWeakFormulation::compute_penalty_parameter(int p, double h) const {
    if (h < 1e-12) return sigma_0_ * (p + 1) * (p + 1);
    ...
    for (int q = 0; q < n_quad; ++q) {
        ...
        for (int i = 0; i < n_basis; ++i) {
            for (int j = 0; j < n_basis; ++j) {
                Eigen::Vector2d grad_phi_i = dphi_dx_vals.block(q * n_basis + i, 0, 1, 2).transpose();
...
LaplaceWeakFormulation::compute_interior_face_integral(
    ...
    for (int i = 0; i < n_face_quad; ++i) {
        ...
        permuted_grad_phi_R.block(i * n_basis, 0, n_basis, 2) = grad_phi_R.block(permutation(i) * n_basis, 0, n_basis, 2);
    }
    ...
    for (int q = 0; q < n_face_quad; ++q) {
        ...
        K_LL += w_q * (-0.5 * (vL * A_vec.transpose()) - 0.5 * (C_vec * uL.transpose()) + sigma * (vL * uL.transpose()));
        K_LR += w_q * (-0.5 * (vL * B_vec.transpose()) + 0.5 * (C_vec * uR.transpose()) - sigma * (vL * uR.transpose()));
        K_RL += w_q * (+0.5 * (vR * A_vec.transpose()) - 0.5 * (D_vec * uL.transpose()) - sigma * (vR * uL.transpose()));
        K_RR += w_q * (+0.5 * (vR * B_vec.transpose()) + 0.5 * (D_vec * uR.transpose()) + sigma * (vR * uR.transpose()));
    }
...
Eigen::MatrixXd LaplaceWeakFormulation::compute_boundary_face_integral(
    ...
    if (bc && bc->get_type() == BCType::DIRICHLET) {
        for (int q = 0; q < n_face_quad; ++q) {
            ...
            K_boundary += w_q * (- (v * C_vec.transpose()) - (C_vec * u.transpose()) + sigma * (v * u.transpose()));
        }
    } else {
        std::cout << "No valid boundary condition provided for element " << elem_id 
                  << " face " << face_id << ". Skipping boundary integral." << std::endl;
    }
...
Eigen::VectorXd LaplaceWeakFormulation::compute_source_integral(
    ...
    if (!source_func) return F_source;
...
Eigen::VectorXd LaplaceWeakFormulation::compute_boundary_rhs_integral(
    int elem_id, int face_id,
    std::shared_ptr<DGMesh> mesh,
    std::shared_ptr<BoundaryCondition> bc) const {
    auto dg_space = mesh->get_dg_space();
    ...
    if (bc->get_type() == BCType::DIRICHLET) {
        ...
        for (int q = 0; q < n_face_quad; ++q) {
            ...
            for (int i = 0; i < n_basis; ++i) {
                ...
                Eigen::Vector2d grad_phi_i = grad_phi.block(q * n_basis + i, 0, 1, 2).transpose();
                F_boundary[i] += w_q * (sigma * bc_value * phi_i - bc_value * grad_phi_i.dot(normal));
            }
        }
    }
...
void LaplaceWeakFormulation::assemble(

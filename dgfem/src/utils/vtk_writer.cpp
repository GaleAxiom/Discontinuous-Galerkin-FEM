...
void VTKWriter::write_solution(std::shared_ptr<DGMesh> mesh, const Eigen::VectorXd& solution,
...
void VTKWriter::write_multi_variable_solution(std::shared_ptr<DGMesh> mesh,
    ...
    for (size_t i = 0; i < solutions.size(); ++i) {
        Eigen::VectorXd values =
            evaluate_at_points(mesh, solutions[i], vis_vertices, resolution_factor);
...
void VTKWriter::write_euler_solution(std::shared_ptr<DGMesh> mesh,
...
void VTKWriter::write_with_analytical(std::shared_ptr<DGMesh> mesh, const Eigen::VectorXd& solution,
    ...
    Eigen::VectorXd numerical_values =
        evaluate_at_points(mesh, solution, vis_vertices, resolution_factor);
...
void VTKWriter::generate_visualization_mesh(std::shared_ptr<DGMesh> mesh, int resolution_factor,
    ...
    Eigen::MatrixXi ref_connectivity =
...
Eigen::VectorXd VTKWriter::evaluate_at_points(std::shared_ptr<DGMesh> mesh,
...
Eigen::MatrixXd VTKWriter::get_reference_subdivision_points(std::string_view element_type,
                                                            int resolution_factor) {
...
Eigen::MatrixXi VTKWriter::get_reference_subdivision_connectivity(std::string_view element_type,
                                                                  int resolution_factor) {
    ...
    if (element_type == "triangle") {
        ...
        for (int j = 0; j < resolution_factor; ++j) {
            for (int i = 0; i < resolution_factor; ++i) {
                Eigen::Vector4i quad;
                quad << quad_index(i, j), quad_index(i + 1, j), quad_index(i + 1, j + 1),
                    quad_index(i, j + 1);
...
void VTKWriter::write_vtk_cells(std::ofstream& file, const Eigen::MatrixXi& connectivity,
...
void VTKWriter::write_vtk_point_data(
    std::ofstream& file, const std::vector<std::pair<std::string, Eigen::VectorXd>>& data_arrays) {

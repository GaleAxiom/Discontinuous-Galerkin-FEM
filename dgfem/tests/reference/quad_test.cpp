...
TEST_F(QuadDGSpaceTest, FaceQuadratureMapping) {
    ...
    std::vector<std::pair<int, std::function<bool(const Eigen::Vector2d&)>>> face_checks = {
        {0, [](const Eigen::Vector2d& xi) { return std::abs(xi[1] + 1.0) < 1e-10; }}, // Bottom: y = -1
        {1, [](const Eigen::Vector2d& xi) { return std::abs(xi[0] - 1.0) < 1e-10; }},  // Right: x = 1
        {2, [](const Eigen::Vector2d& xi) { return std::abs(xi[1] - 1.0) < 1e-10; }},  // Top: y = 1
        {3, [](const Eigen::Vector2d& xi) { return std::abs(xi[0] + 1.0) < 1e-10; }}   // Left: x = -1
    };
...
TEST_F(QuadVolumeIntegralTest, StiffnessMatrix) {
    ...
    for (int i = 0; i < 9; ++i) {
        for (int j = 0; j < 9; ++j) {
            EXPECT_NEAR(K(i, j), K(j, i), 1e-10) << "K(" << i << "," << j << ") != K(" << j << "," << i << ")";
        }
    }

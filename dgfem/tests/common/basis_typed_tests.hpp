...
protected:
    ...
    void testBasisCount() {
        for (int order = 1; order <= getMaxOrder(); ++order) {
            ...
            EXPECT_EQ(n_basis, expected) 
                << "Basis count mismatch for order " << order;
        }
    }
...
protected:
    ...
    void testHConvergence(
        ...
        double expected_l2_rate,
        double expected_h1_rate,
        ...
        for (double h : mesh_sizes) {
            auto [l2_err, h1_err] = solveAndComputeError(
                h, polynomial_order, exact_solution, exact_gradient, source_term
            );
        ...
        double l2_rate = computeConvergenceRate(
            mesh_sizes[0], l2_errors[0],
            mesh_sizes[1], l2_errors[1]
        );
        
        double h1_rate = computeConvergenceRate(
            mesh_sizes[0], h1_errors[0],
            mesh_sizes[1], h1_errors[1]
        );
        
        EXPECT_NEAR(l2_rate, expected_l2_rate, tolerance)
            << "L2 convergence rate " << l2_rate 
            << " differs from expected " << expected_l2_rate;
        
        EXPECT_NEAR(h1_rate, expected_h1_rate, tolerance)
            << "H1 convergence rate " << h1_rate 
            << " differs from expected " << expected_h1_rate;
    }

protected:
    virtual std::pair<double, double> solveAndComputeError(
        double h,
        int order,
        std::function<double(const Eigen::Vector2d&)> exact,
        std::function<Eigen::Vector2d(const Eigen::Vector2d&)> grad,
        std::function<double(const Eigen::Vector2d&)> source
    ) = 0;

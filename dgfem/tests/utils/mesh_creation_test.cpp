...
TEST_F(MeshCreationFixture, CreateRectangularMeshQuads) {
    ...
    std::cout << "DEBUG: About to call create_rectangular_mesh with dx=0.5, use_triangles=false"
              << std::endl;

    // Test quad mesh with default domain [0,1]^2
    auto mesh_quad = MeshCreator::create_rectangular_mesh(0.5, false);

    std::cout << "DEBUG: create_rectangular_mesh returned, mesh pointer = " << mesh_quad.get()
              << std::endl;
    ...
    std::cout << "DEBUG: Vertices size = " << vertices.rows() << " x " << vertices.cols()
              << std::endl;
    ...
    std::cout << "DEBUG: Elements size = " << elements.rows() << " x " << elements.cols()
              << std::endl;
    ...
    EXPECT_EQ(boundary_faces.size(), 8)
        << "Unit square with 2x2 quads should have 8 boundary faces";
...
TEST_F(MeshCreationFixture, CreateEulerMesh) {
    ...
    std::cout << "DEBUG: About to call create_euler_mesh with params: xmin=-5, xmax=5, ymin=-5, "
                 "ymax=5, dx=0.2, use_triangles=false"
              << std::endl;
...
TEST_F(MeshCreationFixture, BoundaryTagsExist) {
    ...
    EXPECT_EQ(boundary_tags.size(), 4)
        << "Should have exactly 4 boundary tags for rectangular domain";
...
TEST(MeshCreationUtilsTest, FindReferenceCoordsQuadCenter) {
    ...
    EXPECT_NEAR((*xi)[0], 0.0, 1e-6)
        << "Center of physical element should map to (0,0) in reference";
    EXPECT_NEAR((*xi)[1], 0.0, 1e-6)
        << "Center of physical element should map to (0,0) in reference";
}

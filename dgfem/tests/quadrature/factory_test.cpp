...
TEST(QuadratureFactoryTest, GaussLegendreQuad) {
    ...
    Eigen::Vector2d expected_points[] = {
        {-p, -p}, {p, -p}, {-p, p}, {p, p}
    };
...
TEST(QuadratureFactoryTest, IntegrationAccuracy) {
    ...
    auto test_polynomial = [](const std::unique_ptr<QuadratureRule>& rule, int poly_deg_x, int poly_deg_y) -> double {
        double integral = 0.0;
        for (int i = 0; i < rule->points.rows(); ++i) {
            double x = rule->points(i,0);
            double y = rule->points(i,1);
    ...
    for (int order = 1; order <= 5; ++order) {
        ...
        if (order == 1) continue; // Order 1 rule is tested separately and is exact for degree 0.
        ...
        double analytical_result = static_cast<double>(factorial(a) * factorial(b)) / factorial(a + b + 2);
...
TEST(QuadratureFactoryTest, DegreeOfPrecision) {
    ...
    auto integrate_monomial_triangle = [](const std::unique_ptr<QuadratureRule>& rule, 
                                          int deg_x, int deg_y) -> double {
        double sum = 0.0;
        for (int i = 0; i < rule->points.rows(); ++i) {
            double x = rule->points(i, 0);
            double y = rule->points(i, 1);
    ...
    auto exact_integral_triangle = [](int a, int b) -> double {
        ...
        for (int i = 1; i <= a; ++i) numerator *= i;
        for (int i = 1; i <= b; ++i) numerator *= i;
        for (int i = 1; i <= a + b + 2; ++i) denominator *= i;
...
TEST(QuadratureFactoryTest, QuadratureConvergence) {
    ...
    EXPECT_LT(errors[3], errors[0]) 
        << "Higher order quadrature not more accurate";
}
...
TEST(QuadratureFactoryTest, ConstantFunctionIntegration) {
    ...
    for (int n = 1; n <= 5; ++n) {
        auto rule = QuadratureFactory::gauss_legendre_quad(n);
        double integral = rule->weights.sum();
        EXPECT_NEAR(integral, 4.0, 1e-14)
            << "Quad constant integration failed for order " << n;
    }
    ...
    for (int n = 1; n <= 5; ++n) {
        auto rule = QuadratureFactory::gauss_legendre_1d(n);
        double integral = rule->weights.sum();
        EXPECT_NEAR(integral, 2.0, 1e-14)
            << "1D constant integration failed for order " << n;
    }
}
...
TEST(QuadratureFactoryTest, LinearFunctionIntegration) {
    ...
    auto integrate_linear_triangle = [](const std::unique_ptr<QuadratureRule>& rule,
...
TEST(QuadratureFactoryTest, QuadraticFunctionIntegration) {
    ...
    auto integrate_quadratic = [](const std::unique_ptr<QuadratureRule>& rule,
    ...
    for (int order = 2; order <= 5; ++order) {
        ...
        for (const auto& [a, b] : test_cases) {
            ...
            EXPECT_NEAR(numerical, analytical, 1e-12)
                << "Order " << order << " failed for quadratic with a=" << a 
                << ", b=" << b;
        }
    }
}

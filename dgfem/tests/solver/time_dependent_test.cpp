...
TEST_F(TimeDependentSolverTest, AdvectionTimeSteppingStability) {
    ...
    auto initial_condition = [](const Eigen::Vector2d& x) -> double {
        ...
        double sigma = 0.1;
        return std::exp(-((x[0]-x0)*(x[0]-x0) + (x[1]-y0)*(x[1]-y0))/(2*sigma*sigma));
    };
    ...
    for (int n = 0; n < n_steps; ++n) {
        ...
        EXPECT_TRUE(std::isfinite(current_norm)) 
            << "Solution should remain finite at step " << n;
...
TEST_F(TimeDependentSolverTest, RK4TimeIntegration) {
    ...
    EXPECT_LT(error, 1e-6) 
        << "RK4 should have very small error for smooth ODE";
}
...
TEST_F(TimeDependentSolverTest, CFLConditionViolation) {
    ...
    EXPECT_GT(dt_unstable, dt_stable) 
        << "Unstable dt should be larger than stable dt";

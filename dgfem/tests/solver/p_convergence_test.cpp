...
TEST_F(PRefinementTest, SmoothSolutionPConvergence) {
    ...
    for (size_t i = 1; i < l2_errors.size(); ++i) {
        EXPECT_LT(l2_errors[i], l2_errors[i-1]) 
            << "L2 error should decrease with increasing p";
        EXPECT_LT(h1_errors[i], h1_errors[i-1]) 
            << "H1 error should decrease with increasing p";
    }
    
    // Exponential convergence means error should drop by at least factor of 2 each time
    EXPECT_LT(l2_errors[1] / l2_errors[0], 0.5) 
        << "p=2 should give at least 2x reduction from p=1";
    EXPECT_LT(l2_errors[2] / l2_errors[1], 0.5) 
        << "p=3 should give at least 2x reduction from p=2";
}
...
TEST_F(PRefinementTest, PolynomialSolutionExactness) {
    ...
    for (int p : orders) {
        ...
        EXPECT_LT(errors["H1"], 1e-8) 
            << "Order " << p << " DG should reproduce degree " << p << " polynomial gradient exactly";
    }
}
...
TEST_F(PRefinementTest, HPRefinementCombination) {
    ...
    std::vector<std::pair<double, int>> hp_pairs = {
        {0.2, 1}, {0.1, 2}, {0.05, 3}
    };
    std::vector<double> l2_errors;

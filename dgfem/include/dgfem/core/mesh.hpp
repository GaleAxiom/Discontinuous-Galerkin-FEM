...
class DGMesh {
public:
    DGMesh(const Eigen::MatrixXd& vertices, 
     ...
     */
    [[nodiscard]] const std::map<std::string, Eigen::VectorXd>& get_face_data(int elem_id, int face_id) const;
    
    /**
     * @brief Get element face data (returns map with MatrixXd for compatibility)
     */
    [[nodiscard]] const std::map<std::string, Eigen::MatrixXd>& get_element_face_data(int elem_id, int face_id) const;
    
    /**
     * @brief Set boundary condition for a boundary tag
     */
    void set_boundary_condition(std::string_view tag_name, 
                               std::shared_ptr<BoundaryCondition> bc);
    
    /**
     * @brief Get boundary condition for element face
     */
    [[nodiscard]] std::shared_ptr<BoundaryCondition> get_boundary_condition(int elem_id, int face_id) const;
     ...
     */
    [[nodiscard]] std::shared_ptr<BoundaryConditionEuler> get_boundary_condition_euler(int elem_id, int face_id) const;
    
    /**
     * @brief Get Euler boundary condition by boundary tag name
     */
    [[nodiscard]] std::shared_ptr<BoundaryConditionEuler> get_boundary_condition_euler(std::string_view tag_name) const;
    ...
    [[nodiscard]] const std::map<std::string, int>& get_boundary_tags() const noexcept { return boundary_tags_; }
     ...
     */
    [[nodiscard]] const std::vector<std::pair<int, int>>& get_boundary_faces() const noexcept { return boundary_faces_; }

    /**
     * @brief Get map from node index to contributing elements
     */
    [[nodiscard]] const std::map<int, std::vector<std::pair<int, int>>>& get_node_to_elements() const noexcept { return node_to_elements_; }
     ...
     */
    [[nodiscard]] const std::vector<FaceConnectivity>& get_interior_faces() const noexcept { return interior_faces_; }

    /**
     * @brief Get precomputed boundary faces
     */
    [[nodiscard]] const std::vector<FaceConnectivity>& get_boundary_face_data() const noexcept { return boundary_face_data_; }
...
private:
     ...
     */
    void identify_boundary_faces(const std::map<int, std::vector<std::pair<int, int>>>& boundary_edges);
    ...
    std::map<int, std::vector<std::pair<int, int>>> node_to_elements_; ///< node_id -> list of (elem_id, local_node_id)

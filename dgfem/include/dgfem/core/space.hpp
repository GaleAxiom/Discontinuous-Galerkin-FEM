...
public:
     ...
     */
    [[nodiscard]] std::map<std::string, Eigen::MatrixXd> compute_element_data(
        const Eigen::MatrixXd& vertices,
        const std::vector<std::pair<int, int>>& face_neighbors = {}
    ) const;
    
    /**
     * @brief Compute face data for element face
     */
    [[nodiscard]] std::map<std::string, Eigen::VectorXd> compute_face_data(
        const Eigen::MatrixXd& vertices,
        int face_id,
        const std::pair<int, int>& neighbor_info = {-1, -1}
    ) const;
    
    /**
     * @brief Get precomputed basis values at volume quadrature points
     */
    [[nodiscard]] const Eigen::MatrixXd& get_volume_basis_values() const noexcept { return phi_vol_; }
    
    /**
     * @brief Get precomputed basis gradients at volume quadrature points  
     */
    [[nodiscard]] const std::vector<Eigen::MatrixXd>& get_volume_basis_gradients() const noexcept { return dphi_vol_; }
    
    /**
     * @brief Get precomputed basis values at face quadrature points
     */
    [[nodiscard]] const std::vector<Eigen::MatrixXd>& get_face_basis_values() const noexcept { return phi_face_; }
    
    /**
     * @brief Compute face permutation for neighboring elements
     */
    [[nodiscard]] Eigen::VectorXi compute_face_permutation(
        int elem1_face, const Eigen::MatrixXd& elem1_vertices,
        int elem2_face, const Eigen::MatrixXd& elem2_vertices
    ) const;
    ...
    [[nodiscard]] std::shared_ptr<ReferenceElement> get_ref_element() const noexcept { return ref_element_; }
    [[nodiscard]] std::shared_ptr<OrthogonalBasis> get_basis() const noexcept { return basis_; }
    [[nodiscard]] std::shared_ptr<QuadratureRule> get_volume_quad() const noexcept { return volume_quad_; }
    [[nodiscard]] std::shared_ptr<QuadratureRule> get_face_quad() const noexcept { return face_quad_; }
    [[nodiscard]] std::shared_ptr<GeometricMapping> get_mapping() const noexcept { return mapping_; }
...
private:
     ...
     */
    [[nodiscard]] std::shared_ptr<OrthogonalBasis> create_basis(std::string_view element_type, int order) const;
    
    /**
     * @brief Create appropriate reference element
     */
    [[nodiscard]] std::shared_ptr<ReferenceElement> create_ref_element(std::string_view element_type) const;
};

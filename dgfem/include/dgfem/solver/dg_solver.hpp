...
public:
     ...
     */
    [[nodiscard]] Eigen::VectorXd solve(std::function<double(const Eigen::Vector2d&)> source_func = nullptr);
...
public:
     ...
     */
    [[nodiscard]] std::vector<Eigen::VectorXd> solve(
        std::function<double(const Eigen::Vector2d&)> initial_condition,
        double T_final,
        double dt,
        std::shared_ptr<BoundaryCondition> boundary_condition = nullptr,
        int save_every = 1);
...
private:
     ...
     */
    [[nodiscard]] Eigen::VectorXd project_initial_condition(std::function<double(const Eigen::Vector2d&)> u0_func);
...
public:
     ...
     */
    [[nodiscard]] std::vector<Eigen::MatrixXd> solve(
...
private:
     ...
     */
    [[nodiscard]] std::vector<Eigen::MatrixXd> apply_mass_inv(const std::vector<Eigen::MatrixXd>& vec) const;
    
    /**
     * @brief Project initial condition using L2 projection
     */
    [[nodiscard]] std::vector<Eigen::MatrixXd> project_initial_condition(
        std::function<Eigen::Vector4d(const Eigen::Vector2d&)> u0_func);
    
    /**
     * @brief Time stepping using SSP-RK3 (Strong Stability Preserving Runge-Kutta 3rd order)
     */
    [[nodiscard]] std::vector<Eigen::MatrixXd> time_step_ssp_rk3(
        const std::vector<Eigen::MatrixXd>& u_n, double dt) const;
     ...
     */
    [[nodiscard]] double compute_max_cfl(const std::vector<Eigen::MatrixXd>& u_coeffs, double dt) const;
};
...
public:
    NavierStokesDGSolver(std::shared_ptr<DGMesh> mesh,
                         double gamma = 1.4,

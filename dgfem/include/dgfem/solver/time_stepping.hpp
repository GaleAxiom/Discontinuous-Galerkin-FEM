...
public:
    ...
    static SolutionType step_rk3(
        const SolutionType& u_n,
        double dt,
        std::function<SolutionType(const SolutionType&)> rhs_func)
    {
        ...
        SolutionType u2 = add_scaled(
            scale(0.75, u_n),
            0.25, add_scaled(u1, dt, k2));
        
        // Stage 3
        SolutionType k3 = rhs_func(u2);
        SolutionType u_np1 = add_scaled(
            scale(1.0/3.0, u_n),
            2.0/3.0, add_scaled(u2, dt, k3));
...
private:
    ...
    static Eigen::VectorXd add_scaled(
        const Eigen::VectorXd& a, double alpha, const Eigen::VectorXd& b)
    {
        return a + alpha * b;
    }
    
    // Helper for scalar vectors: result = alpha*a
    static Eigen::VectorXd scale(double alpha, const Eigen::VectorXd& a) {
        return alpha * a;
    }
    
    // Helper for vector of matrices: result = a + alpha*b
    static std::vector<Eigen::MatrixXd> add_scaled(
    ...
    static std::vector<Eigen::MatrixXd> scale(
        double alpha,
        const std::vector<Eigen::MatrixXd>& a)
    {
        std::vector<Eigen::MatrixXd> result(a.size());
...
public:
     ...
     */
    std::vector<Eigen::MatrixXd> apply_inverse(

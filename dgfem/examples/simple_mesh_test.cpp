...
int main() {
    try {
        ...
        auto mesh = dgfem::MeshCreator::create_rectangular_mesh(
            config.dx, config.use_triangles, 0.0, 1.0, 0.0, 1.0);
        ...
        std::cout << "DG space initialized with " 
                  << dg_space->get_basis()->get_n_basis() 
                  << " basis functions per element" << std::endl;
        
        // Test solution storage
        auto solution = mesh->get_solution();
        std::cout << "Solution storage created with " 
                  << solution->get_total_dofs() << " total DOFs" << std::endl;
        ...
        std::cout << "Gradient dimensions: " << grad_phi.rows() << "x" << grad_phi.cols() << std::endl;
        ...
        std::cout << "Volume weights sum: " << vol_weights_sum 
                  << " (expected: " << expected_measure << ")" << std::endl;

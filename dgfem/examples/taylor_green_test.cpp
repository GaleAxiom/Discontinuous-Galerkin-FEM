...
struct TaylorGreenVortex {
    ...
    TaylorGreenVortex(double rho0_, double U0_, double kx_, double ky_, 
                      double nu_, double gamma_, double p0_)
    ...
    int var_idx)
{
    ...
    for (int e = 0; e < mesh->get_n_elements(); ++e) {
        ...
        for (int q = 0; q < quad_wts.size(); ++q) {
            ...
            error_sq += (W_num[var_idx] - W_exact[var_idx]) * 
                       (W_num[var_idx] - W_exact[var_idx]) * dw;
...
int main() {
    try {
        ...
        auto ic = [&exact](const Eigen::Vector2d& x) { 
            return exact(x[0], x[1], 0.0); 
        };
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            std::cout << "  L2 error (" << std::setw(3) << var_names[i] << "): " 

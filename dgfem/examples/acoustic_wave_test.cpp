...
struct AcousticWave {
    ...
    AcousticWave(double rho0_, double p0_, double u0_, double A, 
                 double wavelength, double gamma_)
    ...
    double exact_phase(double x, double t) const {
        return k * x - omega * t;
    }
};
...
double measure_amplitude(const std::shared_ptr<dgfem::DGMesh>& mesh,
    ...
    for (int i = 0; i < n_samples; ++i) {
        ...
        for (int e = 0; e < n_elem; ++e) {
            ...
            if (x_samples[i] >= x_min && x_samples[i] <= x_max &&
                y_sample >= y_min && y_sample <= y_max) {
...
double measure_phase_shift(const std::shared_ptr<dgfem::DGMesh>& mesh,
    ...
    for (int i = 0; i < n_samples; ++i) {
        ...
        num_fourier += weight * std::complex<double>(
            rho_num_samples[i] * std::cos(k * x),
            -rho_num_samples[i] * std::sin(k * x)
        );
        
        exact_fourier += weight * std::complex<double>(
            rho_exact_samples[i] * std::cos(k * x),
...
// Compute L2 errors
std::map<std::string, double> compute_errors(
    const std::shared_ptr<dgfem::DGMesh>& mesh,
...
int main() {
    try {
        ...
        std::cout << "  Wave amplitude: " << amplitude << " (" << (100.0 * amplitude / rho0) << "% of background)" << std::endl;
        ...
        std::cout << "  Time step dt: " << dt << " (exactly " << steps_per_period << " steps per period)" << std::endl;
        ...
        for (size_t r_idx = 0; r_idx < ppw_values.size(); ++r_idx) {
            ...
            std::cout << "  CFL number: " << dt * c0 * (2.0 * config.order + 1.0) / config.dx << std::endl;
            ...
            std::cout << "  DOFs: " << mesh->get_n_elements() * dg_space->get_basis()->get_n_basis() * 4 << std::endl;
            ...
            std::vector<Eigen::MatrixXd> solutions = solver.solve(
                initial_condition, T_final, dt, save_every);
            ...
            double initial_amplitude = measure_amplitude(mesh, solutions[0], exact_wave.k, gamma, rho0);
            ...
            for (size_t i = 0; i < solutions.size(); ++i) {
                ...
                results["dissipation"].push_back((initial_amplitude - amp) / initial_amplitude * 100.0);
            ...
            // Print summary
            std::cout << "\n  Time    | Amplitude | Dissipation | Phase Error | L2 Error" << std::endl;
            std::cout << "  " << std::string(65, '-') << std::endl;
            for (size_t i = 0; i < results["time"].size(); ++i) {
                std::cout << "  " << std::setw(6) << std::fixed << std::setprecision(3) << results["time"][i]
                         << "  | " << std::setw(9) << std::scientific << std::setprecision(3) << results["amplitude"][i]
                         << " | " << std::setw(10) << std::fixed << std::setprecision(2) << results["dissipation"][i] << "%"
                         << " | " << std::setw(11) << std::scientific << std::setprecision(2) << results["phase_error"][i]
                         << " | " << std::setw(8) << std::setprecision(2) << results["l2_error"][i] << std::endl;
            }
            
            // Export VTK
            for (size_t i = 0; i < solutions.size(); ++i) {
                std::string filename = "../../output/acoustic_" + resolution_name + "_" + std::to_string(i);
        ...
        std::cout << std::string(60, '=') << std::endl;
        std::cout << "\nNOTE: All tests use SAME wavelength with DIFFERENT mesh resolutions." << std::endl;
        ...
        for (size_t r = 0; r < ppw_values.size(); ++r) {
            ...
            std::cout << "\nResolution: " << resolution_names[r] << " (PPW = " << actual_ppw[r] << "):" << std::endl;
            ...
            std::cout << "  Average L2 error: " << std::setprecision(3) << avg_l2_error << std::endl;
            std::cout << "  Maximum L2 error: " << std::setprecision(3) << max_l2_error << std::endl;
            std::cout << "  Final L2 error: " << std::setprecision(3) << final_l2_error << std::endl;
